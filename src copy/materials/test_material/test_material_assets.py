...
(function() {
    ...
    function initQuizSystem() {
        // Initialize single choice quizzes
        initSingleChoiceQuizzes();
        // Initialize categorization quizzes
        initCategorizationQuizzes();
        // Initialize multiple choice quizzes (these are handled by window.checkMultipleChoice, so no specific init here)
    }

    function initSingleChoiceQuizzes() {
        const singleChoiceQuizzes = document.querySelectorAll('.single-choice-quiz');
        singleChoiceQuizzes.forEach(quiz => {
            // No specific event listeners needed here, as checkSingleChoice is called by button click
        });
    }
    ...
    function initCategorizationQuizzes() {
        ...
        categorizationQuizzes.forEach(quiz => {
            ...
            const dropZones = quiz.querySelectorAll('.drop-zone'); // Changed to .drop-zone

            // ドラッグ可能なアイテムのイベントリスナー
            draggableItems.forEach(item => {
                item.addEventListener('dragstart', function(e) {
                    // Use the actual element's ID for data transfer
            ...
            // ドロップゾーンのイベントリスナー
            dropZones.forEach(zone => { // Changed to .drop-zone
                zone.addEventListener('dragover', function(e) {
                    e.preventDefault();
                    zone.classList.add('drag-over'); // Apply to drop-zone
                });

                zone.addEventListener('dragleave', function(e) {
                    // Check if the related target is outside the drop zone
                    if (!zone.contains(e.relatedTarget)) {
                        zone.classList.remove('drag-over'); // Apply to drop-zone
                    }
                });
                ...
                zone.addEventListener('drop', function(e) {
                    ...
                    if (draggedElement) {
                        // Find the original pool of draggable items
                        const initialDraggableItemsPool = quiz.querySelector('.draggable-items');

                        // If the dragged element is currently in the initial pool, remove it from there
                        if (initialDraggableItemsPool && initialDraggableItemsPool.contains(draggedElement)) {
                            ...
                            // If it's in another drop-area, remove it from there
    ...
    window.checkSingleChoice = function(quizId) {
        ...
        const quizData = window.singleChoiceData ? window.singleChoiceData[quizId] : null;
        ...
        let isCorrect = (selectedIndex === quizData.correct);
