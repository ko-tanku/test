...
def main():
    ...
    if content_config_path.exists():
        with open(content_config_path, 'r', encoding='utf-8') as f:
            config_data = yaml.safe_load(f)

    # --- 2b. ナビゲーション構造の処理 ---
    nav_structure = config_data.get('navigation', [])
    
    def process_nav_placeholders(nav_item, material_name):
        if isinstance(nav_item, dict):
            for key, value in nav_item.items():
                if isinstance(value, str):
                    nav_item[key] = value.replace('{{ material_name }}', material_name)
                elif isinstance(value, list):
                    process_nav_placeholders(value, material_name)
        elif isinstance(nav_item, list):
            for item in nav_item:
                process_nav_placeholders(item, material_name)
